 ...
 *
 * Eleven of the twelve layout components pick the block types they understand
 * out of the card (`blocksOfTypeIndexed`) and render only those. Anything else
 * on the card was drawn nowhere — while the PPTX export walks
 ...
 *
 * Each layout now names the indices it drew and renders the remainder, which
 * turns "does this layout account for every block?" into a property of one
 * pure function instead of eleven hand-written JSX trees. There are no
 * component tests in this repo by design, so being pure is what makes the
