...
- **Addendum (found during manual verification, not anticipated above):** removing the code path that *creates* anonymous sessions does not invalidate anonymous sessions *already sitting in a browser's `localStorage`* from before this feature shipped. Left unhandled, `RequireAuth` would treat such a leftover session as authenticated and skip the login screen entirely — a real violation of "nobody can reach the app's pages without a real account." Fixed in `authStore.ts`: session state resolution treats `session.user.is_anonymous === true` the same as no session at all, for both the initial `getSession()` hydration and the `onAuthStateChange` subscription.
